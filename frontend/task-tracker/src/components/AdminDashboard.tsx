...
export default function AdminDashboard({
  ...
  const profileNames =
    (profiles
      ?.map((p: Profile) => p.full_name || p.name)
      .filter(Boolean) as string[]) ?? [];
  const taskNames = [
    ...new Set(tasks.map((t: Task) => t.responsible).filter(Boolean)),
  ];
  const names =
  ...
  const stats = names.map((name) => {
    const mine = tasks.filter((t: Task) => t.responsible === name);
    const done = mine.filter((t: Task) =>
      ["Completed", "Ontime", "Completed Delay"].includes(t.status),
    ).length;
    return {
      name,
      total: mine.length,
      today: mine.filter((t: Task) => t.status === "TodayTask").length,
      tomorrow: mine.filter((t: Task) => t.status === "Tomorrow").length,
      pending: mine.filter((t: Task) => t.status === "Pending").length,
      overdue: mine.filter((t: Task) => t.status === "Overdue").length,
      completed: done,
      pct: mine.length ? Math.round((done / mine.length) * 100) : 0,
    };
  });

  const totalOverdue = tasks.filter((t: Task) => t.status === "Overdue").length;
  const totalCompleted = tasks.filter((t: Task) =>
    ["Completed", "Ontime", "Completed Delay"].includes(t.status),
  ).length;
  const totalToday = tasks.filter((t: Task) => t.status === "TodayTask").length;
                    ...
                    >
                      <div
                        ...
                        style={{
                          background:
                            AVATAR_COLORS[
                              s.name as keyof typeof AVATAR_COLORS
                            ] || "#64748b",
                          width: 28,
                          height: 28,
                          fontSize: 11,
