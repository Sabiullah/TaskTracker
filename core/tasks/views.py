...
class TaskViewSet(UidLookupMixin, ModelViewSet):
    ...
    def perform_create(self, serializer):
        user = cast(User, self.request.user)
        task = serializer.save(created_by=user, org=getattr(user, "org", None))
    ...
    def delete_all(self, request):
        ...
        deleted, _ = Task.objects.filter(org=user_org).delete()
    ...
    def bulk_create(self, request):
        ...
        for row in rows:
            ...
            if s.is_valid():
                s.save(created_by=request.user, org=user_org)
...
class TaskLogViewSet(UidLookupMixin, ModelViewSet):
    ...
    def get_queryset(self):
        qs = TaskLog.objects.select_related("task", "changed_by").order_by("-changed_at")
