...
def backfill(apps, schema_editor):
    ...
    plan_by_key = {
        (p.main_task_id, p.subcategory_id): p.pk for p in Plan.objects.filter(subcategory__isnull=False).iterator()
    }
    # Children link to their plan via (parent_id, category_id). Rows whose
    # pair has no plan (manual/legacy one-offs) stay plan=NULL.
    for child in Task.objects.filter(parent__isnull=False, category__isnull=False, plan__isnull=True).iterator():
