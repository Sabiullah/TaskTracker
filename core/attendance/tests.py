...
class WfhApprovalTests(TestCase):
    ...
    def test_approvals_pending_for_manager(self):
        ...
        Attendance.objects.create(
            user=self.emp, org=self.org, date=dt.date(2026, 4, 28),
            status="Present", work_location="WFH",
            login_time=dt.time(9), logout_time=dt.time(18),
            approval_state="Pending",
        )
    ...
    def test_patch_cannot_change_approval_state(self):
        ...
        Attendance.objects.create(
            user=self.emp, org=self.org, date=dt.date(2026, 4, 29),
            status="Present", work_location="WFH",
            login_time=dt.time(9), logout_time=dt.time(18),
            approval_state="Pending",
        )
        ...
        c = self._client(self.emp)
        r = c.patch(f"/api/attendance/{row.uid}/", {"approval_state": "Approved"}, format="json")
